...
class ForexIndicatorAnalyzer:
    ...
    def create_retrieval_chain(self, collection):
        ...
        # 创建提示模板，指导模型对内容进行分析
        template = """
        You are a professional trader and quantitative analyst with 20 years of experience. Your task is to extract key information needed to recreate technical indicators from ForexFactory forum discussions. Based on the following content:

        {context}

        Please extract information about technical indicators mentioned in the discussion, and output in the strict JSON format below:

        ```json
        [
          {{
            "Indicator": "Indicator Name (in English)",
            "Usage": "Detailed description of calculation methods, parameter settings and usage tips (in Chinese)"
          }}
        ]
        ```

        When analyzing indicators, please provide:
        1. Complete and accurate indicator name in English
        2. The usage section (in Chinese) should include:
           - Calculation method or mathematical formula of the indicator
           - Key parameter settings and their default or recommended values
           - How to interpret the indicator and its trading signals
           - Special cases to note during implementation
           - Any details that would help a programmer recreate this indicator

        Here are format examples:

        ```json
        [
          {{
            "Indicator": "MACD",
            "Usage": "接收收盘价序列，按默认快速期12、慢速期26、信号期9计算MACD线（快速EMA减慢速EMA）、信号线（MACD线的EMA）和柱状图（两线差值）。当MACD线上穿信号线时发出看多信号，下穿时发出看空信号；柱状图的高度和方向反映动量强弱。"
          }},
          {{
            "Indicator": "RSI",
          ...
          {{
            "Indicator": "CDLDOJI",
            "Usage": "接收开盘、最高、最低、收盘价序列，检测"十字星"形态——开盘价与收盘价几乎相等的烛线。当该模式出现时返回非零整数，否则返回0；用于识别市场犹豫状态，常在趋势末端或反转前出现，需要后续烛线确认。"
          }}
        ]
        ```

        Important notes:
        1. If certain implementation details are missing from the discussion, please note "未提供详细实现方法" in the "用法" field
        2. Prioritize indicators with clear implementation details
        3. Ensure the output is valid JSON format
        4. If no valid technical indicator information is found, return an empty array []
        5. Focus on helping the next model successfully recreate these indicators
        6. IMPORTANT: Keep the indicator name in English but ALL usage descriptions MUST be in Chinese
        """
    ...
    def analyze_indicators(self):
        ...
        try:
            ...
            with open(output_path, "w", encoding="utf-8") as f:
                f.write(f"# 技术指标分析报告\n\n")
                f.write(f"## 总览\n共计划处理 {total_collections} 个thread\n\n")
            ...
            updated_content = content.replace(
                f"共计划处理 {total_collections} 个thread", 
                f"共处理 {total_collections} 个thread，其中新处理 {processed_collections} 个，跳过 {skipped_collections} 个"
            )
            ...
            print(f"总计: 新处理 {processed_collections} 个集合，跳过 {skipped_collections} 个集合")
    ...
    def analyze(self):
        ...
        # 执行指标分析
        output_path = self.analyze_indicators()
        
        # 如果分析过程出错或所有集合都已分析过
        if not output_path:
            latest_summary = self._get_latest_summary_file()
            print("未生成新报告，返回之前的报告路径")
