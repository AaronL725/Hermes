...
class BaseLogicFactors:
    ...
    @staticmethod
    @nb.njit
    def HIGH(high, open, low, close, vol, oi):
        """
        Calculate the Average True Range (ATR) for each security in the time series data.

        Inputs:
            high (np.ndarray): An array of high prices with shape (time_steps, securities).
            open (np.ndarray): An array of opening prices with shape (time_steps, securities).
            low (np.ndarray): An array of low prices with shape (time_steps, securities).
            close (np.ndarray): An array of closing prices with shape (time_steps, securities).
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).

        Outputs@1:
            np.ndarray: An array of ATR values with shape (time_steps, securities).
        """
        return high
    @staticmethod
    @nb.njit
    ...
    @staticmethod
    @nb.njit
    def low(high, open, low, close, vol, oi):
        """
        Calculate the Average True Range (ATR) for each security in the time series data.
    ...
    def ATR(high, open, low, close, vol, oi, timeperiod=14):
        ...
        Inputs:
            ...
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).
            N (int): The number of periods to calculate the ATR over. Default is 14.

        Outputs@1:
            np.ndarray: An array of ATR values with shape (time_steps, securities).
        @@:
            @N1_=[14,28,56,74,100,170,250]
        """
    ...
    @staticmethod
    @nb.njit
    def BIAS(high, open, low, close, vol, oi, N1_=6):
        """
        Calculate the Bias, which is the distance from the moving average.
        ...
        Inputs:
            ...
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).
            N1_ (int): The number of periods for the moving average and Bias calculation. Default is 6.

        Outputs@1:
            np.ndarray: An array of Bias values with shape (time_steps, securities).
        @@:
            @N1_=[5,6,10,20,44,56,74,100,144,250]
        """
        tdts, secs = high.shape
        Tfactors = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        for sec in range(secs):
            for ts in range(tdts):
                if ts <=N1_ or close[ts,sec]!=close[ts,sec]:
                    continue
                ...
                _oi = oi[:ts + 1, sec]
                # if ts % 10000 == 0:
                #     print(sec, ts,"calBIAS")


                myhigh = getavailabledata(_high, N1_)
                myclose = getavailabledata(_close, N1_)
                mylow = getavailabledata(_low, N1_)
                myvol = getavailabledata(_vol, N1_)
                myoi = getavailabledata(_oi,N1_)
                _maN = ROLLING_MEAN(myclose,N1_)
    ...
    @staticmethod
    @nb.njit
    def VolPriceCorr(high, open, low, close, vol, oi, N1_=20):
        """
        Calculate the correlation between volume and price for each security in the time series data.
        ...
        Inputs:
            ...
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).
            N1_ (int): The number of periods to consider for the calculation. Default is 20.

        Outputs@1:
            np.ndarray: An array of VolPriceCorr values with shape (time_steps, securities).
        @@:
            @N1_=[5,10,20,30,40,60,100,250]
        """
        tdts, secs = high.shape
        Tfactors = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        for sec in range(secs):
            for ts in range(tdts):
                ...
                _oi = oi[:ts + 1, sec]
                # if ts % 10000 == 0:
                #     print(sec, ts,"calVolPriceCorr")

                myopen = getavailabledata(_open, N1_)
                myhigh = getavailabledata(_high, N1_)
                myclose = getavailabledata(_close, N1_)
                mylow = getavailabledata(_low, N1_)
                myvol = getavailabledata(_vol, N1_)
    ...
    @staticmethod
    @nb.njit
    def MA(high,open,low,close,vol,oi,N1_=20):
        """
        Calculate the moving average (MA) for each security in the time series data.
        ...
        Inputs:
            ...
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).
            N1_ (int): The number of periods to calculate the MA over. Default is 20.

        Outputs@1:
            np.ndarray: An array of moving average values with shape (time_steps, securities).
        @@:
            @N1_=[5,10,20,30,44,56,74,100,144,250]
        """
        tdts, secs = high.shape
        Tfactors = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        for sec in range(secs):
    ...
    def MACD(high, open, low, close, vol, oi, fastperiod=12, slowperiod=26, signalperiod=9):
        """
        Calculate the Moving Average Convergence Divergence (MACD) for each security in the time series data.
        ...
        Inputs:
            ...
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).
            N1_ (int): The number of periods for the long EMA. Default is 26.
            N2_ (int): The number of periods for the signal line (DEA). Default is 9.
            N3_ (int): The number of periods for the short EMA. Default is 12.

        Outputs@3:
            tuple: A tuple containing three arrays - DIF, DEA, and MACD with shapes (time_steps, securities), (time_steps, securities), and (time_steps, securities) respectively.
        @@:
            @N1_=[26]
            @N2_=[9]
            @N3_=[12]
    ...
    @staticmethod
    @nb.njit
    def KDJ(high, open, low, close, vol, oi, N1_=9, N2_=3, N3_=3):
        """
        Calculate the Stochastic Oscillator (KDJ) for each security in the time series data.
        ...
        Inputs:
            ...
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).
            N1_ (int): The number of periods for the RSV calculation. Default is 9.
            N2_ (int): The number of periods for the K line smoothing. Default is 3.
            N3_ (int): The number of periods for the D line smoothing. Default is 3.

        Outputs@3:
            tuple: A tuple containing three arrays - K, D, and J with shapes (time_steps, securities), (time_steps, securities), and (time_steps, securities) respectively.
        ...
        for sec in range(secs):
            for ts in range(tdts):
                ...
                rsv = (myclose - TSMIN(mylow, N1_)) / (TSMAX(myhigh, N1_) - TSMIN(mylow, N1_)) * 100
                _k = ROLLING_EWM(rsv, 2 / (N2_ + 1))
                _d = ROLLING_EWM(_k, 2 / (N3_ + 1))
                _j = 3 * _k - 2 * _d
                k[ts, sec] = _k[-1]
                d[ts, sec] = _d[-1]
                j[ts, sec] = _j[-1]
    ...
    @staticmethod
    @nb.njit
    def BOLL(high, open, low, close, vol, oi, N1_=20, N2_=2):
        """
        Calculate the Bollinger Bands for each security in the time series data.
        ...
        Inputs:
            ...
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).
            N1_ (int): The number of periods for the long EMA. Default is 20.
            N2_ (int): The number of periods for the standard deviation calculation. Default is 2.

        Outputs@3:
            tuple: A tuple containing three arrays - up, middle, and down with shapes (time_steps, securities).
        ...
        """
        tdts, secs = high.shape
        up = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        middle = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        down = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        for sec in range(secs):
            for ts in range(tdts):
                if ts <= N1_ or close[ts,sec] != close[ts,sec]:
                    continue
                ...
                _mid = MEAN(myclose)
                _std  = STD(myclose)
                up[ts,sec] = _mid + N2_ * _std
                down[ts,sec] = _mid - N2_ * _std
                middle[ts,sec]  = _mid
    ...
    @staticmethod
    @nb.njit
    def RSI(high,open,low,close,vol,oi,N1_=14):
        """
        Calculate the Relative Strength Index (RSI) for each security in the time series data.
        ...
        Inputs:
            ...
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).
            N (int): The number of periods for the RSI calculation. Default is 14.

        Outputs@1:
            tuple: A tuple containing one array - rsi with shape (time_steps, securities).

        @@:
            @N1_=[7,14,25,30,45]
        """
        tdts, secs = high.shape
        rsi = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        for sec in range(secs):
            for ts in range(tdts):
                if ts <=N1_ or close[ts,sec]!=close[ts,sec]:
                    continue
                ...
                N = N1_
                myopen = getavailabledata(_open,N+1)
                myhigh = getavailabledata(_high,N+1)
                myclose = getavailabledata(_close, N+1)
                mylow = getavailabledata(_low, N+1)
                myvol = getavailabledata(_vol, N+1)
                myoi = getavailabledata(_oi,N+1)

                close_diff = myclose - DELAY(myclose,1)
                up = np.maximum(close_diff,0)
                down = ABS(np.minimum(close_diff,0))
                up_ma = ROLLING_MEAN(up,N)
                down_ma = ROLLING_MEAN(down,N)
                _rs = up_ma[-1]/down_ma[-1]  if down_ma[-1]!=0 else np.nan
                _rsi = 100 - (100/(1+_rs))
                if np.isinf(_rsi):
                    _rsi = 100
                if np.isnan(_rsi):
    ...
    def ADX(high, open, low, close, vol, oi, timeperiod=14):
        """
        Calculate the ADX for each security in the time series data.
    ...
    @staticmethod
    @nb.njit
    def ContinueYangK(high,open,low,close,vol,oi,N1_=1,N2_=1):
        """
        Calculate the Yang K Channel breakout signal for each security in the time series data.
        ...
        Inputs:
            ...
            vol (np.ndarray): An array of volume data with shape (time_steps, securities).
            oi (np.ndarray): An array of open interest data with shape (time_steps, securities).
            N1_ (int): The number of periods for the RSI calculation. Default is 1.
            N2_ (int): The number of periods to check after a breakout. Default is 1.

        Outputs@1:
            tuple: A tuple containing one array - YangK with shape (time_steps, securities).
        @@:
            @N1_=[1,2,3,4,5]
            @N2_=[1,2,3,4,5]
        """
        tdts, secs = high.shape
        YangK = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
    ...
    @staticmethod
    @nb.njit
    def ContinueYingK(high,open,low,close,vol,oi,N1_=1,N2_=1):
        """
        Calculate the Ying K Channel breakout signal for each security in the time series data.
        ...
        tdts, secs = high.shape
        YingK = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
    ...
    @staticmethod
    @nb.njit
    def ContinueUpK1(high,open,low,close,vol,oi,N1_=1,N2_=1):
        """
        Calculate the ContinueUpK Channel breakout signal for each security in the time series data.
        ...
        tdts, secs = high.shape
        ContinueUpK1 = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        ...
        for sec in range(secs):
            for ts in range(tdts):
                ...
                myopen = getavailabledata(_open,N1_+1)
                myhigh = getavailabledata(_high,N1_+1)
                myclose = getavailabledata(_close, N1_+1)
                mylow = getavailabledata(_low, N1_+1)
                myvol = getavailabledata(_vol, N1_+1)
                myoi = getavailabledata(_oi,N1_+1)
                _n = 0
                for i in range(N1_,0,-1):
                    if myclose[i]>myclose[i-1] and myopen[i]>myopen[i-1]:
                        _n +=1
                if _n == N1_:
                    for i in range(N2_):
                        if ts+i < tdts:
                            ContinueUpK1[ts+i,sec] = 1
    ...
    @staticmethod
    @nb.njit
    def ContinueUpK2(high,open,low,close,vol,oi,N1_=1,N2_=1):
        """
        Calculate the ContinueUpK Channel breakout signal for each security in the time series data.
        ...
        tdts, secs = high.shape
        ContinueUpK2 = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        ...
        for sec in range(secs):
            for ts in range(tdts):
                ...
                _n = 0
                for i in range(N1_,0,-1):
                    if myhigh[i]>myhigh[i-1] and mylow[i]>mylow[i-1]:
                        _n +=1
                if _n == N1_:
                    for i in range(N2_):
                        if ts+i < tdts:
                            ContinueUpK2[ts+i,sec] = 1
    ...
    @staticmethod
    @nb.njit
    def ContinueDownK1(high,open,low,close,vol,oi,N1_=1,N2_=1):
        """
        Calculate the ContinueDownK1 Channel breakout signal for each security in the time series data.
        ...
        tdts, secs = high.shape
        ContinueDownK1 = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        ...
        for sec in range(secs):
            for ts in range(tdts):
                ...
                if _n == N1_:
                    for i in range(N2_):
                        if ts+i < tdts:
                            ContinueDownK1[ts+i,sec] = 1

        return  ContinueDownK1


    
    @staticmethod
    @nb.njit
    def ContinueDownK2(high,open,low,close,vol,oi,N1_=1,N2_=1):
        """
        Calculate the ContinueDownK2 Channel breakout signal for each security in the time series data.
        ...
        tdts, secs = high.shape
        ContinueDownK2 = np.array([np.float64(np.nan)] * (tdts * secs)).reshape(tdts, secs)
        ...
        for sec in range(secs):
            for ts in range(tdts):
                ...
                if _n == N1_:
                    for i in range(N2_):
                        if ts+i < tdts:
                            ContinueDownK2[ts+i,sec] = 1

        return  ContinueDownK2
    ...
    #     """
    #     Calculate Candlestick Doji Pattern,The indicator is used to identify the Doji candlestick pattern. When the opening price and the closing price are very close (the body is less than a certain percentage of the candle's range), the Doji pattern is formed.
    #     Purpose: It is used to identify the state of market indecision, which may signal a trend reversal.
    #     Inputs:
    #         high (np.ndarray): An array of high prices with shape (time_steps, securities).
    #         open (np.ndarray): An array of opening prices with shape (time_steps, securities).
    #         low (np.ndarray): An array of low prices with shape (time_steps, securities).
    ...
    def ADOSC(high, open, low, close, vol, oi, fast_period=3, slow_period=10):
        """
        Calculate ADOSC breakout signal for each security in the time series data.
    ...
    def ADXR(high, open, low, close, vol, oi, timeperiod=14):
        """
        Calculate ADXR for each security in the time series data.
    ...
    def AROON(high, open, low, close, vol, oi, timeperiod=14):
        """
        Calculate AROON for each security in the time series data.
    ...
    def AROONOSC(high, open, low, close, vol, oi, timeperiod=14):
        """
        Calculate AROONOSC for each security in the time series data.
    ...
    def BBANDS(high, open, low, close, vol, oi, timeperiod=20, nbdevup=2.0, nbdevdn=2.0, matype=0):
        """
        Calculate BBANDS for each security in the time series data.
    ...
    def BETA(high, open, low, close, vol, oi, timeperiod=5):
        """
        Calculate BETA for each security in the time series data.
    ...
    def BOP(high, open, low, close, vol, oi):
        """
        Calculate Balance Of Power Calculation method: (Closing price - Opening price) / (Highest price - Lowest price) Purpose: To measure the balance of the buying and selling power in the market
    ...
    #     """
    #     Calculate    CDLDRAGONFLYDOJI - Dragonfly Doji Candlestick Pattern
    ...
    #     """
    #     Calculate  CDLHIGHWAVE - Candlestick High Wave Pattern
    ...
    def CDLHIKKAKE(high, open, low, close, vol, oi):
        """
        Calculate  CDLHIKKAKE - Candlestick Hikkake Pattern
    ...
    def CDLSPINNINGTOP(high, open, low, close, vol, oi, body_period=10):
        """
        Calculate   CDLSPINNINGTOP - Candlestick Spinning Top Pattern
    ...
    def CDLSTICKSANDWICH(high, open, low, close, vol, oi, equal_period=3):
        """
        Calculate   CDLSTICKSANDWICH - Candlestick Stick Sandwich Pattern
    ...
    def WCLPRICE(high, open, low, close, vol, oi):
        """
        Calculate  WCLPRICE - Weighted Close Price
